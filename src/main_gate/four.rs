...
impl<F: FieldExt> MainGateInstructions<F> for MainGate<F> {
    ...
    fn combine(
        ...
        term_0.constrain_equal(region, cell_0)?;
        term_1.constrain_equal(region, cell_1)?;
        term_2.constrain_equal(region, cell_2)?;
        term_3.constrain_equal(region, cell_3)?;
    ...
    fn assign_value(
        ...
        let cell =
            region.assign_advice(|| "assign value", column, *offset, || unassigned.value())?;
...
mod tests {
    ...
    use crate::{Assigned, AssignedCondition, UnassignedValue};
        ...
        fn synthesize(
            ...
            layouter.assign_region(
                ...
                |mut region| {
                    let _ = region.assign_advice(
                        || "a",
                        config.main_gate_config.a,
                        0,
                        || Ok(F::from_u64(10)),
                    )?;
                    let _ = region.assign_advice(
                        || "b",
                        config.main_gate_config.b,
                        0,
                        || Ok(F::from_u64(20)),
                    )?;
                    let _ = region.assign_advice(
                        || "c",
                        config.main_gate_config.c,
                        0,
                        || Ok(F::from_u64(30)),
                    )?;
                    let _ = region.assign_advice(
                        || "d",
                        config.main_gate_config.d,
                        0,
                        || Ok(F::zero()),
                    )?;
                    ...
                    region.assign_fixed(
                        || "sc",
                        config.main_gate_config.sc,
                        0,
                        || Ok(-F::one()),
                    )?;
                    region.assign_fixed(
                        || "sd",
                        config.main_gate_config.sd,
                        0,
                        || Ok(F::zero()),
                    )?;
                    region.assign_fixed(
                        || "s_mul",
                        config.main_gate_config.s_mul,
                        0,
                        || Ok(F::zero()),
                    )?;
                    region.assign_fixed(
                        || "sd_next",
                        config.main_gate_config.sd_next,
                        0,
                        || Ok(F::zero()),
                    )?;
                    region.assign_fixed(
                        || "s_constant",
                        config.main_gate_config.s_constant,
                        0,
                        || Ok(F::zero()),
                    )?;
            ...
            layouter.assign_region(
                ...
                |mut region| {
                    ...
                    region.assign_fixed(
                        || "sa",
                        config.main_gate_config.sa,
                        0,
                        || Ok(F::zero()),
                    )?;
                    region.assign_fixed(
                        || "sb",
                        config.main_gate_config.sb,
                        0,
                        || Ok(F::zero()),
                    )?;
                    region.assign_fixed(
    ...
    fn test_main_gate() {
        ...
        let circuit = TestCircuitMainGate::<Fp> {
            _marker: PhantomData,
        };
        ...
        fn synthesize(
            ...
            layouter.assign_region(
                ...
                |mut region| {
                    ...
                    let a = main_gate.assign_value(
                        &mut region,
                        &UnassignedValue::new(self.a),
                        super::MainGateColumn::A,
                        &mut offset,
                    )?;
                    let b = main_gate.assign_value(
                        &mut region,
                        &UnassignedValue::new(self.b),
                        super::MainGateColumn::A,
                        &mut offset,
                    )?;
                    ...
                    let must_be_false =
                        main_gate.is_equal(&mut region, a.clone(), b.clone(), &mut offset)?;
                    let must_be_true =
                        main_gate.is_equal(&mut region, a.clone(), a.clone(), &mut offset)?;
                    ...
                    let zero = main_gate.assign_value(
                        &mut region,
                        &UnassignedValue::new(zero),
                        super::MainGateColumn::A,
                        &mut offset,
                    )?;
        ...
        fn synthesize(
            ...
            layouter.assign_region(
                ...
                |mut region| {
                    ...
                    let c_0 = main_gate.assign_value(
                        &mut region,
                        &c,
                        super::MainGateColumn::A,
                        &mut offset,
            ...
            layouter.assign_region(
                ...
                |mut region| {
                    ...
                    let cond: AssignedCondition<F> = main_gate
                        .assign_value(&mut region, &cond, super::MainGateColumn::A, &mut offset)?
                        .into();
                    let selected =
                    ...
                    let b_assigned = main_gate.assign_value(
                        &mut region,
                        &b_unassigned,
                        super::MainGateColumn::A,
                        &mut offset,
